...
save_dir = '/scratch/ca2699/DDPG-MFG/0906_experiment_results/our_algorithm'
...
class CyberSecEnv(gym.Env):
  ...
  def get_mu_and_reward_defender(self, mu, alpha):
      ...
      for it in range(0,self.Nt_inner_loop):
          ...
          print('this is the time step for defenders',it)
          print('this is the social reward for defenders',social_reward)
          new_mu_prev = new_mu
  ...
  def get_mu_and_reward_attacker(self, mu_attackers, alpha_attackers):
      ...
      for it in range(0,self.Nt_inner_loop):
          ...
          print('the distribution of attackers is',new_mu_attackers)
          social_reward_att -= self.running_cost_t_attackers(0, new_mu_attackers)*self.Dt_inner_loop
          print('the current timestep is',it)
          print('the attacker social reward is',social_reward_att)
  ...
  def step(self, action):
    ...
    print('the social reward is',social_reward)
    self.t += self.dt
    done = (self.t > self.T)
    info = None
    ...
    def update(
        ...
        # See Pseudo Code.
          ...
          with tf.GradientTape() as tape:
              ...
              #print(next_state_def_batch)
              target_actions_def = target_actor_def([next_state_def_batch, next_state_att_batch], training=True)
              ...
              y = reward_def_batch + gamma * target_critic_def(
                  [next_state_def_batch, next_state_att_batch, target_actions_def], training=True
              )
              critic_def_value = critic_model_def([state_def_batch, state_att_batch, action_def_batch], training=True)
          ...
          with tf.GradientTape() as tape:
              actions_def = actor_model_def([state_def_batch, state_att_batch], training=True)
              critic_def_value = critic_model_def([state_def_batch, state_att_batch, actions_def], training=True)
          ...
          with tf.GradientTape() as tape:
              target_actions_att = target_actor_att([next_state_def_batch, next_state_att_batch], training=True)
              #print("target_actions = ", target_actions_2)
              y = reward_att_batch + gamma * target_critic_att(
                  [next_state_def_batch, next_state_att_batch, target_actions_att], training=True
              )
              critic_att_value = critic_model_att([state_def_batch, state_att_batch, action_att_batch], training=True)
          ...
          with tf.GradientTape() as tape:
              ...
              actions_att = actor_model_att([state_def_batch, state_att_batch], training=True)
              critic_att_value = critic_model_att([state_def_batch, state_att_batch, actions_att], training=True)
              # Used `-value` as we want to maximize the value given
              # by the critic for our actions
...
def policy(state_def, state_att,  actor_model, noise_object, action_lower_bound, action_upper_bound):
...
def compute_maxJ(object_actor_model_1, object_critic_model_1, object_target_actor_1, object_target_critic_1, target_actor_2,
 ...
 #emporarily change to 1 for debugging
  for i in range(100):
    ...
    pop_distrib_reset = distrib_init_list[0]
    ...
    while True:
      ...
      if object_index == 0:
          action_object = policy(tf_prev_state_def, tf_prev_state_att, actor_model_object, ou_noise_defenders, action_lower_bound, action_upper_bound)
      elif object_index == 1:
          action_object = policy(tf_prev_state_def, tf_prev_state_att, actor_model_object, ou_noise_attackers, action_lower_bound, action_upper_bound)

      #print('the previous state is', prev_state)
      action_1 = policy(tf_prev_state_def, tf_prev_state_att, target_actor_2, ou_noise_zero, action_lower_bound, action_upper_bound)
    ...
    if (i%50)==0:
      ...
      plt.legend()
      save_path = os.path.join(save_dir, "The_First_distribution_defenders_inner_DDPG_episode{}_train-distrib.pdf".format(i))
      ...
      save_path = os.path.join(save_dir,"The_First_actions_defenders_inner_DDPG_episode{}_train-distrib.pdf".format(i) )
      plt.savefig(save_path)
      plt.show()
      plt.clf()
      ...
      plt.ylabel("distribution")
      plt.legend()
      save_path = os.path.join(save_dir, "The_First_distribution_attacker_inner_DDPG_episode{}_train-distrib.pdf".format(i))
      plt.savefig(save_path)
      plt.show()
      plt.clf()
      ...
      save_path = os.path.join(save_dir, "The_First_actions_attacker_inner_DDPG_episode{}_train-distrib.pdf".format(i))
      plt.savefig(save_path)
      plt.show()
      plt.clf()
  ...
  plt.tight_layout()
  if object_index == 0:
    save_path = os.path.join(save_dir,'The_First_calculate_exploitability_exploitability_inner_reward_defender'+'reward-training_inner_ddpg_episodde{}.pdf'.format(ep) )
    plt.savefig(save_path)
  elif object_index == 1:
    save_path = os.path.join(save_dir, 'The_First_calculate_exploitability_exploitability_inner_reward_attacker'+'reward-training_inner_ddpg_episodde{}.pdf'.format(ep))
    plt.savefig(save_path)
  plt.show()
  plt.clf()
...
def compute_J_i (target_actor_1,
  ...
  for i in range(eval_times):
      ...
      while True:
        ...
        action_test_1 = policy(tf_prev_test_state_1, tf_prev_test_state_2, target_actor_1, ou_noise_zeros, action_lower_bound, action_upper_bound)
        action_test_2 = policy(tf_prev_test_state_1, tf_prev_test_state_2, target_actor_2, ou_noise_zeros, action_lower_bound, action_upper_bound)
...
def train_policy(training_env, actor_model_def, actor_optimizer_def, critic_model_def, critic_optimizer_def, target_actor_def, target_critic_def,
  ...
  for ep in range(N_total_episodes+1):
    ...
    while True: # loop over time steps in the current episode
        ...
        action_def = policy(tf_prev_state_def, tf_prev_state_att,  actor_model_def  , ou_noise_defenders, action_lower_bound_def, action_upper_bound_def)
        action_att = policy(tf_prev_state_def, tf_prev_state_att,  actor_model_att  , ou_noise_attackers, action_lower_bound_att, action_upper_bound_att)
    ...
    if (ep % 50 == 0):
      # Plot result of latest training step
          ...
          plt.ylabel("distribution")
          plt.legend()
          save_path = os.path.join(save_dir,"The_First_calculate_exploitability_distribution_defenders_episode{}_train-distrib.pdf".format(ep) )
          plt.savefig(save_path)
          plt.show()
          plt.clf()
    ...
    if (ep % 10 == 0):
      ...
      plt.ylabel("reward")
      save_dir = '/scratch/ca2699/DDPG-MFG/0906_experiment_results/our_algorithm'
      save_path = os.path.join(save_dir,"The_First_rewards_episode{}.pdf".format(ep))
      plt.savefig(save_path)
      plt.show()
      plt.clf()
      ...
      if (ep%4==0):
        for i in range(len(actor_lr_lst_def)):
          ...
          for j in range(len(sdv_lst_def)):
              ...
              output_dir = '/scratch/ca2699/DDPG-MFG/0906_experiment_results/our_algorithm/logs'
    ...
    # 写入多个数据点
              ...
              save_path = os.path.join(save_dir,'The_First_calculate_exploitability_exploitability_defenders_episode{}.pdf'.format(ep) )
              plt.savefig(save_path)
              plt.show()
              plt.clf()
              ...
              #os.makedirs(save_dir, exist_ok=True)
              #plt.savefig(save_path)
              save_path = os.path.join(save_dir, 'The_First_calculate_exploitability_exploitability_attackers_episode{}.pdf'.format(ep))
              plt.savefig(save_path)
              plt.show()
              plt.clf()
              print('this is the init population distribution list 8',distrib_init_list[0])
...
#tune the parameters
actor_lr_lst_def = [ 0.006]
critic_lr_lst_def = [ 0.009]
sdv_lst_def = [0.08]
...
#distrib_init_list = [[[0.35, 0.35, 0.15, 0.15],[0.3,0.7]], [[0.45, 0.35, 0.15, 0.05],[0.2,0.8]], [[0.15, 0.25, 0.25, 0.35],[0.7,0.3]], [[0.25, 0.15, 0.35, 0.25],[0.55,0.45]], [[0.5, 0.2, 0.2, 0.1],[0.3,0.7]]]
#distrib_init_list = [[[0.8, 0.05, 0.05, 0.1],[0.3,0.7]], [[0.1, 0.7, 0.1, 0.1],[0.2,0.8]], [[0.1, 0.1, 0.7, 0.1],[0.7,0.3]], [[0.5, 0.0, 0.5, 0.0],[0.55,0.45]], [[0.5, 0.2, 0.2, 0.1],[0.3,0.7]]]
distrib_init_list = [[[0.8, 0.05, 0.05, 0.1],[0.3,0.7]]]
